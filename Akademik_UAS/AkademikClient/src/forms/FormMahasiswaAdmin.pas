...
begin
  ...
  // 🔥 RESET GRID DENGAN BENAR
  sgMahasiswa.RowCount := Arr.Count + 2;
  InitGrid; // header SETELAH RowCount
...
begin
  ...
  begin
    ShowMessage('Pilih data mahasiswa dulu');
    Exit;
  end;
...
begin
  ...
  if MessageDlg('Hapus mahasiswa ini?', mtConfirmation,
    [mbYes, mbNo], 0) = mrYes then
  begin
    TApiClient.Get(
      '/admin/mahasiswa_delete.php?id_mahasiswa=' + SelectedID.ToString
    );
...
begin
  if ARow = 0 then Exit; // jangan header
  ...
  cbKelas.Text := sgMahasiswa.Cells[3, ARow];
